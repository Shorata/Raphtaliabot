	...
	// Filter Banned People
        ...
        case 'ownermenu'
         if (!isOwnerBot) return aruga.reply(from, 'Perintah ini hanya untuk Owner bot!', id)
            if (args.length == 0) return aruga.reply(from, menuId.textOwner())
        case 'donate':
        case 'donasi':
        ...
        case 'ownerbot':
            ...
            .then(() => aruga.sendText(from, 'Jika kalian ingin membicarakan sesuatu dengan owner silakan chat'))
            break
        case 'join':
            if (args.length == 0) return aruga.reply(from, `Jika kalian ingin mengundang saya kegroup silahkan invite atau dengan\nketik ${prefix}join [link group]`, id)
            ...
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          ...
                          await aruga.sendText(chekgrup.id, `Hai minna~, Saya Raphtalia Bot, Saya adalah bot gratis yang akan selalu melayani anda tapi sebagai gantinya saya akan membagikan berita harian yang dibuat oleh tim kami. Adil bukan?, silakan ketik ${prefix}menu untuk melihat fitur apa saja yang bisa saya berikan ^^`)
                      })
            } else {
                let cgrup = await aruga.getAllGroups()
                if (cgrup.length > groupLimit) return aruga.reply(from, `Maaf namun group yang bisa saya masuki sudah mencapai batas yaitu: ${groupLimit} silakan chat owner bot agar ia bisa memperbanyak group limitnya ^^`, id)
                if (cgrup.size < memberLimit) return aruga.reply(from, `Maaf Namun persyaratan minimal anggota yang berada di group tidak memenuhi syarat T^T. Syarat agar bot bisa masuk adalah jumlah anggota minum yaitu : ${memberLimit} Orang silakan undang lebih banyak orang ke groupmu! agar saya bisa melayani kalian ^^`, id)
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () =>{
	...
	case 'cooltext':
        ...
        case 'stiker':
            ...
            case 'antistiker':
                    ...
                    if (!isGroupMsg) return aruga.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
                    if (!isGroupAdmins) return aruga.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
                    if (!isBotGroupAdmins) return aruga.reply(from, 'Wahai para admin!, jadikanlah saya admin dulu ^_-', id)
                    if (args[0] == 'on') {
                        var cek = antilink.includes(chatId);
                        if(cek){
                    ...
                    break  
    case 'botinfo'
            if (!isGroupMsg) return aruga.reply(from, .menuId.textinfo())
    case 'raphilink'
            if (isGroupMsg) return aruga.reply(from, menuId.textlink())
	case 'brainly':
    ...
    if (!isGroupMsg) return aruga.reply(from, menuId.textPrem())
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *R A P H T A L I A  B C* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *R A P H T A L I A   B C* 〙\n\n${msg}`)
            }
            aruga.reply(from, 'Broadcast Success!', id)

        case 'bcnews': //untuk broadcast atau promosi
            if (!isOwnerBot) return aruga.reply(from, 'Perintah ini hanya untuk Owner bot!', id)
            if (args.length == 0) return aruga.reply(from, `Untuk broadcast ke semua chat ketik:\n${prefix}bcnews [isi chat]`)
            let msg = body.slice(4)
            const chatz = await aruga.getAllChatIds()
            for (let idk of chatz) {
                var cvk = await aruga.getChatById(idk)
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *R A P H T A L I A  N E W S* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *R A P H T A L I A   N E W S* 〙\n\n${msg}`)
            }
            aruga.reply(from, 'Broadcast Success!', id)
            break
